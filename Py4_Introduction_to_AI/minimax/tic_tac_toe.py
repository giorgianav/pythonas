    ...
    def min_value(self, curr_board, last_pos):
        ...
        for pos in curr_board:
            ...
            if curr_board[pos] == " ":
                ...
                sim_utility = self.max_value(deepcopy(curr_board), pos)
                # TODO 4.1.2. Compute the worst utility as the  minimum between the sim_utility and worst_utility
                worst_utility = min(sim_utility, worst_utility)

        print("MIN worst utility = ", worst_utility)
    ...
    def max_value(self, curr_board, last_pos):
        ...
        for pos in curr_board:
            ...
            if curr_board[pos] == " ":
                ...
                sim_utility = self.min_value(deepcopy(curr_board), pos)
                # TODO 7.1.2. the best_utility is the max between the current best and the utility after
                # the simulation performed on this subtree
                best_utility = max(sim_utility, best_utility)

        print("MAX best utility = ", best_utility)
        return best_utility
    ...
    def alphabeta(self):
        ...
        for pos in self.board:
            ...
            if self.board[pos] == " ":
                ...
                s = self.minABValue(curr_board, pos, alpha, beta)
                # TODO 8.1.3. if the result is better than our best score so far, save that move and score
                if s > score:
                    # if the result is better than our best score so far, save that move and score
                    move = pos
                    score = s
    ...
    def maxABValue(self, curr_board, last_pos, alpha, beta):
        ...
        for pos in curr_board:
            ...
            if curr_board[pos] == " ":
                ...
                score = max(score, self.minABValue(nextBoard, pos, alpha, beta))
